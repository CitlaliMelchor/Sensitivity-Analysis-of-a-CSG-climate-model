...
if any(sign(delta_t(1))*delta_t <= 0)
   error('Entries of time vector "t" are not correct.');
                        ...
                        % note: time steps can be non-equidistant
...
% with time step defined as: delta_t (vector!)
for i = 1:N-1
                        % compute dx/dt (or k) with function defined in fh
   k1 = feval(fh, t(i),              x(:,i));
   k2 = feval(fh, t(i)+delta_t(i)/2, x(:,i)+k1*delta_t(i)/2);
   k3 = feval(fh, t(i)+delta_t(i)/2, x(:,i)+k2*delta_t(i)/2);
   k4 = feval(fh, t(i)+delta_t(i),   x(:,i)+k3*delta_t(i));

   x(:,i+1) = x(:,i) + delta_t(i)*(k1+2*k2+2*k3+k4)/6;
end
